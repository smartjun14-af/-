...
def dashboard_page():
    ...
    with ui.column().classes('w-full min-h-screen bg-[#F8FAFC] text-[#1E293B] p-8 gap-6'):
        ...
        with ui.row().classes('gap-6 w-full items-start no-wrap'):

                    # ===================== 왼쪽: 시세 차트 (크게, 2/3) =====================
                    with ui.card().classes('grow bg-white text-[#1E293B] border border-[#E2E8F0] rounded-2xl shadow-lg p-6'):
                        ui.label('시세 차트').classes('text-2xl font-bold text-[#1E293B]')

                        selected_coin = {'value': 'BTC'}
                        selected_interval = {'value': 'day'}

                        def fetch_candles(token: str, coin: str, interval: str) -> go.Figure:
                            candles = []
                            try:
                                result = api_client.get_candles(token, coin=coin, interval=interval, count=60)
                                candles = result.get('candles', [])
                            except Exception as e:
                                print(f"[캔들 조회 실패] {coin} {interval}: {e}")

                            if not candles:
                                fig = go.Figure()
                                fig.update_layout(
                                    paper_bgcolor='#EFF6FF', plot_bgcolor='#EFF6FF',
                                    font=dict(color='#2563EB'),
                                    annotations=[dict(text='데이터 없음', showarrow=False, font=dict(size=16))]
                                )
                                return fig
                            ...
                            fig = go.Figure(data=[go.Candlestick(
                                ...
                                increasing_line_color='#D24F45',   # 양봉(상승): 빨강 (한국식)
                                decreasing_line_color='#1261C4',   # 음봉(하락): 파랑
                                increasing_fillcolor='#D24F45',
                                decreasing_fillcolor='#1261C4',
                                name=coin,
                            )])
                            fig.update_layout(
                                ...
                                xaxis=dict(type='category', rangeslider=dict(visible=False),
                                        showgrid=True, gridcolor='#F1F5F9', tickangle=-45, nticks=8),
                                yaxis=dict(showgrid=True, gridcolor='#F1F5F9', tickformat=',', side='right'),
                                showlegend=False,
                                height=480,   # ← 차트 높이 키움
                            )
                            return fig
                        ...
                        with ui.row().classes('gap-2 mb-2'):
                            for c in ['BTC', 'ETH', 'XRP', 'SOL']:
                                def make_coin_handler(coin=c):
                                    async def handler():
                                        selected_coin['value'] = coin
                                        for sym, btn in coin_buttons.items():
                                            if sym == coin:
                                                btn.classes(remove='bg-[#DBEAFE] text-[#2563EB]')
                                                btn.classes(add='bg-[#2563EB]')
                                                btn.style('color:#FFFFFF !important;')
                                            else:
                                                btn.classes(remove='bg-[#2563EB]')
                                                btn.classes(add='bg-[#DBEAFE] text-[#2563EB]')
                                                btn.style('color:#2563EB !important;')
                                        await update_chart()
                                    return handler
                                is_selected = (c == 'BTC')
                                btn_class = 'bg-[#2563EB] font-bold rounded-lg px-4' if is_selected else 'bg-[#DBEAFE] border border-[#BFDBFE] font-bold rounded-lg px-4'
                                btn_style = 'color:#FFFFFF !important;' if is_selected else 'color:#2563EB !important;'
                                b = ui.button(c, on_click=make_coin_handler(c)).classes(btn_class).style(btn_style)
                                coin_buttons[c] = b

                        # ── 인터벌 버튼 ──
                        interval_map = {'1분': 'minute1', '10분': 'minute10', '30분': 'minute30', '1시간': 'minute60', '일봉': 'day'}
                        interval_buttons = {}
                        with ui.row().classes('gap-2 mb-4'):
                            for label, val in interval_map.items():
                                def make_interval_handler(v=val, l=label):
                                    async def handler():    # ← async 로 변경
                                        selected_interval['value'] = v
                                        for lbl, btn in interval_buttons.items():
                                            if lbl == l:
                                                btn.classes(remove='bg-[#DBEAFE] text-[#2563EB]')
                                                btn.classes(add='bg-[#2563EB]')
                                                btn.style('color:#FFFFFF !important;')
                                                ...
                                                btn.classes(add='bg-[#DBEAFE] text-[#2563EB]')
                                                btn.style('color:#2563EB !important;')
                                        await update_chart()    # ← await 추가
                                    return handler
                                is_sel = (label == '일봉')
                                bc = 'bg-[#2563EB] font-bold rounded-lg px-3' if is_sel else 'bg-[#DBEAFE] border border-[#BFDBFE] font-bold rounded-lg px-3'
                                bs = 'color:#FFFFFF !important;' if is_sel else 'color:#2563EB !important;'
                                ib = ui.button(label, on_click=make_interval_handler(val, label)).classes(bc).style(bs)
                                interval_buttons[label] = ib

                        # ── 차트 위젯 ──
                        chart_fig = fetch_candles(get_token(), 'BTC', 'day')
                        chart = ui.plotly(chart_fig).classes('w-full')

                        async def update_chart():
                            token = get_token()
                            fig = await run.io_bound(fetch_candles, token, selected_coin['value'], selected_interval['value'])
                            chart.figure = fig
                            chart.update()
                        
                        async def refresh_last_candle():
                            token = get_token()
                            try:
                                fresh = await run.io_bound(api_client.get_dashboard, token)
                                coins_data = fresh.get('coins', [])
                                current = next((c for c in coins_data if c['coin'] == selected_coin['value']), None)
                                if not current:
                                    return

                                price = current['price']
                                # 마지막 캔들 close만 현재가로 업데이트
                                chart.figure.data[0].close = list(chart.figure.data[0].close[:-1]) + [price]
                                chart.figure.data[0].high = list(chart.figure.data[0].high[:-1]) + [
                                    max(chart.figure.data[0].high[-1], price)
                                ]
                                chart.figure.data[0].low = list(chart.figure.data[0].low[:-1]) + [
                                    min(chart.figure.data[0].low[-1], price)
                                ]
                                chart.update()
                            except Exception:
                                pass

                        ui.timer(0.5, refresh_last_candle)   #마지막 캔들만 갱신
                    ...
                    with ui.column().classes('w-96 gap-6'):
                        ...
                        with ui.card().classes('w-full bg-white text-[#1E293B] border border-[#E2E8F0] rounded-2xl shadow-lg p-6'):
                            ...
                            coin_labels = {}
                            for item in coins:
                                rate_color = '#EF4444' if item['rate'] >= 0 else'#2563EB'
                            ...
                            async def refresh_prices():
                                ...
                                try:
                                    ...
                                    for coin in fresh.get('coins', []):
                                        lbl = coin_labels.get(coin['coin'])
                                        if not lbl:
                                            continue
                                        lbl['price'].set_text(format_won(coin['price']))
                                        color = '#EF4444' if coin['rate'] >= 0 else'#2563EB'
                                        lbl['rate'].set_text(f"{coin['rate']:+.2f}%")
                                        lbl['rate'].style(f'color:{color};')
                                except Exception:
                                    pass
